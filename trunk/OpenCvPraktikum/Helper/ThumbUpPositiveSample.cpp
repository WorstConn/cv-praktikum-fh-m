/* 
 * File:   ThumbUpPositiveSample.cpp
...
ThumbUpPositiveSample::ThumbUpPositiveSample() {
}

ThumbUpPositiveSample::ThumbUpPositiveSample(const ThumbUpPositiveSample& orig) : CreationBehavior(orig) {
}
 ...
 *          
 */
String ThumbUpPositiveSample::createImageInfo(Mat& img, String imgPath, int pos) {
    ...
    for (vector<Point>::const_iterator it = contour.begin(); it != contour.end(); it++) {
        ...
        if (p.x <= imageWidth && p.x > 0) {
            if (maxX.x == 0 || p.x > maxX.x) {
                if ((((float) p.y) / ((float) inputImgF64.size().height)) < 0.85f) {
                    maxX = p;
                }
            }
    ...
    int tmpY = minY.y + static_cast<int> (((float) tmpW)*(13.0f / 17.0f));


    if (minX.y > tmpY) {
        DBG("Neue Anpassung notendig: %s", imgPath.c_str());
        tmpY = minY.y + static_cast<int> (1.75f * MyMath::abs((float) (minX.y - minY.y)));
    } else if (maxX.y > tmpY) {
         DBG("Neue Anpassung notendig: %s", imgPath.c_str());
        tmpY=maxX.y;
    }
 ...
 *          
 */
void ThumbUpPositiveSample::createImageInfo(ArrayOfStringArrays input, String output, StringArray backgroundImagePath, bool createMarkedOutputFiles, bool presegmentedData) {
