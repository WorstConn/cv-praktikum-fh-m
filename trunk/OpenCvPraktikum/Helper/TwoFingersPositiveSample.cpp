/* 
 * File:   TwoFingersPositiveSample.cpp
...
TwoFingersPositiveSample::TwoFingersPositiveSample() {
}

TwoFingersPositiveSample::TwoFingersPositiveSample(const TwoFingersPositiveSample& orig) : CreationBehavior(orig) {
}
 ...
 *          
 */
String TwoFingersPositiveSample::createImageInfo(Mat& img, String imgPath, int pos) {
    ...
    int tmpY = minY.y + tmpW;


    int tmpY2 = (minX.y > maxX.y) ? MyMath::abs(minX.y - minY.y) : MyMath::abs(maxX.y - minY.y);
    tmpY2 = tmpY2 + static_cast<int> (((float) tmpY2)*0.35f);
    tmpY2 += minY.y;
    if (tmpY2 > tmpY) {
        x4 = Point(minX.x, tmpY2);
        x3 = Point(maxX.x, tmpY2);
    } else {
 ...
 *          
 */
void TwoFingersPositiveSample::createImageInfo(ArrayOfStringArrays input, String output, StringArray backgroundImagePath, bool createMarkedOutputFiles, bool presegmentedData) {
